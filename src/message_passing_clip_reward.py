...
from rewards.rewards import RewardGraph
...
def evaluate_graph_set(set_name, graphs, scorer, image, image_feat, weights):
    ...
    baseline_mp_reward = float(baseline_R.detach().cpu())
    ...
    for graph_name, graph, full_prompt in graphs:
        ...
        delta_hier = reward - baseline_mp_reward
        delta_clip = clip_score - baseline_clip_score

        print(f"MP reward: {reward:+.4f} (delta vs {baseline_name}: {delta_hier:+.4f})")
...
if __name__ == "__main__":
    ...
    ref1_image_path = resolve_image_path(data_dir, ["ref1.png", "exact.png"])
    ref2_image_path = resolve_image_path(data_dir, ["ref2.png", "exact.png", "ref1.png"])
