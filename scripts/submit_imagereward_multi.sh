...
# ── Multi-prompt training regime ──────────────────────────────────────────
# Train on the paper-style multi-prompt set, value function on, multiple
# seeds per prompt at eval time.
...
if python -u src/dpok_imagereward.py \
    ...
    # Multi-prompt eval with many seeds per prompt — each prompt sees the
    # training signal only a fraction of the time, so this acts as both
    # in-distribution and light generalization check.
    echo ""
    echo "════════════════════════════════════════════════════════════════"
    echo "  Eval: training prompts × $SEEDS_PER_PROMPT seeds — $(date)"
    echo "════════════════════════════════════════════════════════════════"
    if eval_run python -u src/eval_report.py \
        ...
        --metrics          $METRICS \
        --seed             42 \
        --run_name         "DPOK multi-prompt"; then
        EVAL_STATUS="OK"
    else
        EVAL_STATUS="FAILED"
    fi
