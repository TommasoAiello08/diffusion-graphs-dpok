#!/usr/bin/env python3
"""
eval_report.py — Pretty evaluation report for Diffusion Graphs.

Generates a clean, publication-ready evaluation report including:
  - CLIPScore (ViT-L-14)
  - VQAScore (CLIP-FlanT5-XXL)       — optional, needs t2v-metrics
  - DSGScore (regex decomposition)    — optional, needs VQAScore
  - T2I-CompBench (object/attr/rel)   — optional, needs VQAScore

Outputs:
  <save_dir>/
    report.txt          — Formatted table for terminal / paper
    report.json         — Machine-readable scores
    report.tex          — LaTeX table ready for paper
    per_image_scores.csv — Per-image breakdown
    score_distributions.png
    score_radar.png     — Radar chart of all metrics

Usage:
  # CLIPScore only (fast, ~2 min):
  python eval_report.py \\
      --lora_path dpok_outputs/coco/lora_unet_final \\
      --coco OpenPSG/data/coco/annotations/captions_train2017.json \\
      --max_prompts 50 --save_dir eval_report
  ...
  python eval_report.py \\
      --coco OpenPSG/data/coco/annotations/captions_train2017.json \\
      --max_prompts 50 --save_dir eval_report/baseline
"""
...
def parse_args():
    ...
    p.add_argument("--prompts_file", type=str, default=None,
                   help="JSON list of prompts (paper-style eval on training prompts). "
                        "If set, --coco/--psg/--gqa are ignored. Each prompt is rendered "
                        "with `--seeds_per_prompt` seeds.")
    p.add_argument("--seeds_per_prompt", type=int, default=10,
                   help="When --prompts_file is set, how many independent seeds per prompt.")
    p.add_argument("--coco",        type=str, default=None)
    p.add_argument("--psg",         type=str, default=None)
    p.add_argument("--gqa",         type=str, default=None)
    ...
    p.add_argument("--save_dir",    type=str, default="eval_report")
    p.add_argument("--run_name",    type=str, default=None,
                   help="Name for this run in the report (e.g. 'COCO LoRA')")
    return p.parse_args()
...
def format_report(
    ...
    lines.append("")
    lines.append("=" * W)
    lines.append(f"  DIFFUSION GRAPHS — EVALUATION REPORT")
...
def main():
    ...
    if args.prompts_file:
        ...
        # multiple independent samples per prompt — matches the paper's setting
        # of evaluating on the training prompts with many seeds.
        ...
        items = []
        if args.coco:
            items += load_coco_items(args.coco, max_captions=args.max_prompts, seed=args.seed)
        if args.psg:
            items += load_psg_items(args.psg, max_scenes=args.max_prompts)
        if args.gqa:
            items += load_gqa_items(args.gqa, max_scenes=args.max_prompts)
        random.seed(args.seed)
        random.shuffle(items)
        items   = items[:args.max_prompts]
        prompts = [it.text for it in items]
    else:
        raise ValueError("Provide --prompts_file or --coco/--psg/--gqa")
